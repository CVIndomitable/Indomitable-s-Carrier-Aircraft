...
- [x] 弹药类型：机枪弹匣、航空炸弹、航空鱼雷、航空火箭弹；弹匣支持剩余弹量与部分补给
- [x] 分弹种弹道：自由落体航弹、制导航弹、入水自走鱼雷、直线火箭
- [x] 火控系统：最多4个实体/坐标目标，支持集火/均衡分配
- [x] 自动锁定：最近、威胁最高、集火、分散、类型过滤
- [x] 编组基础：玩家飞机索引、长机指定、简单编组、盘旋点等齐出发、全机召回
- [x] 长机支持：长机视角切换、长机周围 3×3 区块强制加载、终端显示加载区块数量
- [x] 反潜探测：反潜机周期性高亮水下目标（时效 Glowing）
- [x] 控制终端 GUI：小地图、打击坐标、配置、编队、盘旋、召回、补给、长机视角
- [x] 控制数据持久化：玩家设置、盘旋点、长机、编组保存到 SavedData
